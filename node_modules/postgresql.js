...
var port = 5432;
var user = "postgres";
var database = "idros-development-database";
...
var insertWeather = function(zipCode, dateweather, conditions, temperature, precipitation, humidity) {
	...
	client.connect(function(err) {
  		...
  		var query = client.query("WITH upsert AS (UPDATE weathers SET conditions =$3, temperature = $4, precipitation = $5, humidity = $6 WHERE weather_date=$2 AND zip_code = $1 RETURNING *) INSERT INTO weathers (zip_code, weather_date, conditions, temperature, precipitation, humidity) SELECT $1, $2, $3, $4, $5, $6 WHERE NOT EXISTS (SELECT * FROM upsert);", [zipCode, dateweather, conditions, temperature, precipitation, humidity], function(err, result) {
			if(err) {
...
var insertCityWeather = function(cityAndCountry, dateweather, conditions, temperature, precipitation, humidity) {
	...
	var client = new pg.Client(conString);
        client.connect(function(err) {
                ...
                var query = client.query("WITH upsert AS (UPDATE weathers SET conditions =$3, temperature = $4, precipitation = $5, humidity = $6 WHERE weather_date=$2 AND city = $1 AND country = $7 RETURNING *) INSERT INTO weathers (city, country, weather_date, conditions, temperature, precipitation, humidity) SELECT $1, $7,$2, $3, $4, $5, $6 WHERE NOT EXISTS (SELECT * FROM upsert);", [city, dateweather, conditions, temperature, precipitation, humidity, country], function(err, result) {
                        if(err) {
                                console.error('error running query', err);
                                return client.end();
                        }
...
var getCities = function(callback){
	...
	var cities = client.connect(function(err){
		...
		var query = client.query("SELECT city.alias AS city, country.alias AS country from city, country, units where city.idcountry IS NOT NULL AND city.idcountry != 227 AND city.idcountry = country.idcountry AND city.idcity = unit.idcity;");
		var array = [];
		query.on("row", function (row, result) {
			var city = row["city"];
...
var insertSchedule = function(datetime, duration, idzone, callback){
	...
	var insert = client.connect(function(err){
		if(err || datetime == null ) {
			...
			var query = client.query("INSERT INTO schedules (id_zone, start_date, is_automatic, duration, is_restriction, remaining_time, status) VALUES ($1, $2, $3, $4, FALSE, $4, $5); ", [idzone, datetime, true, duration, 'PENDING'], function(err, result) {
							if(err) {
									//console.log(query);
									console.error('error running query', err);
...
var getSchedulesByDate = function(date, unit, callback){
    ...
    var scheduleAux = client.connect(function(err){
        ...
        var query = client.query("SELECT DISTINCT ON (start_date) * FROM schedules, units, zones WHERE start_date BETWEEN $1 AND $2 AND zones.id_unit = $3 AND zones.id = schedules.id_zone ORDER BY start_date DESC",[startDate, endDate, unit]);
        //console.log(query);
        query.on("row", function (row, result) {
...
var getIrrigationNeed = function(idSoilType, idGrassType, callback){
    ...
    var getIrrigation = client.connect(function(err){
		...
		var irrigationNeed;
		var query = client.query("SELECT irrigation_need FROM soil_grass_system_data where id_soil_type = $1 and id_plants_type = $2",[idSoilType, idGrassType]);
		query.on("row", function (row, result) {
...
var getZonesWithoutSensors = function(callback){
    ...
    client.connect(function(err) {
        ...
        var query = client.query("SELECT id from zones where id_sensor is NULL");
        if(err){
            console.error('error running query', err);
            callback(err, null);
        }
